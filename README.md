...
### Install
    ...
    ansible-playbook -i inventories/opensearch/hosts opensearch.yml --extra-vars "opensearch_admin_password=Test@123 opensearch_kibanaserver_password=Test@6789"

You should set the reserved users(`admin` and `kibanaserver`) password using `opensearch_admin_password` and `opensearch_kibanaserver_password` variables.

If you define your own internal users (in addition to the reserved `admin` and `kibanaserver`) in custom configuration
files, then passwords to them should be set via variables on the principle of `opensearch_<username>_password`

It will install and configure the opensearch. Once the deployment completed, you can access the opensearch Dashboards with user `admin` and password which you provided for variable `opensearch_admin_password`.

    # Deploy with ansible playbook - run the playbook as non-root user which have sudo privileges,
    ansible-playbook -i inventories/opensearch/hosts opensearch.yml --extra-vars "opensearch_admin_password=Test@123 opensearch_kibanaserver_password=Test@6789" --become
